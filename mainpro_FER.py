...
#modified by HY, perform in "resume" mode directly
#if opt.resume:
    # Load checkpoint.
...
#modified by HY, perform "resume" mode directly
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        #output = model(images)
        #####
        diff = utils2.orth_dist(net.layer2[0].shortcut[0].weight) + utils2.orth_dist(net.layer3[0].shortcut[0].weight) + utils2.orth_dist(net.layer4[0].shortcut[0].weight)
        '''diff=0
        model=net'''
        diff += utils2.deconv_orth_dist(net.layer1[0].conv1.weight, stride=1) + utils2.deconv_orth_dist(net.layer1[1].conv1.weight, stride=1)
        diff += utils2.deconv_orth_dist(net.layer2[0].conv1.weight, stride=2) + utils2.deconv_orth_dist(net.layer2[1].conv1.weight, stride=1)
        diff += utils2.deconv_orth_dist(net.layer3[0].conv1.weight, stride=2) + utils2.deconv_orth_dist(net.layer3[1].conv1.weight, stride=1)
        diff += utils2.deconv_orth_dist(net.layer4[0].conv1.weight, stride=2) + utils2.deconv_orth_dist(net.layer4[1].conv1.weight, stride=1)
        #####
        
        loss = criterion(outputs, targets) + 0.5 * diff
        #add by HY
        
        
        
        #delete by HY  loss = criterion(outputs, targets)
        loss.backward()
